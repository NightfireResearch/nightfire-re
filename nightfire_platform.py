...
class NightfirePlatform:
    ...
    def dump_iso_if_known(self, iso_file: str, hash_value: str) -> tuple[bool, str | None, PlatformHash | Any]:
        handler = None
    ...
    def extract_and_expand_game_files(
            self, dump_folder: str, platform_hash: PlatformHash, skip_driving: bool, skip_action: bool):
        if not skip_action:
    ...
    def _extract_and_expand_driving_files(self, dump_folder: str, platform_hash: PlatformHash):
        ...
        # driving_handler.dump_driving_files(dump_folder, platform_hash)

    def _extract_and_expand_eurocom_files(self, dump_folder: str, platform_hash: PlatformHash):
        eurocom_handler = None
        if "ps2" in self.current_platform.lower():
            eurocom_handler = PlaystationEurocomHandler()
        if "gamecube" in self.current_platform.lower():
            eurocom_handler = GameCubeEurocomHandler()
        if "xbox" in self.current_platform.lower():
            eurocom_handler = XboxEurocomHandler()

        if eurocom_handler is None:
            logger.warning("No eurocom handler configured for %s", self.current_platform)
            return

        eurocom_handler.dump_eurocom_files(dump_folder, platform_hash)
